...
export const registerForEvent    = async (req: AuthRequest, res: Response) => { try { const { guests = [] } = req.body; res.status(201).json(await RegistrationService.register(req.params.id, req.user!.userId, guests)) } catch (e: any) { res.status(400).json({ message: e.message }) } }
